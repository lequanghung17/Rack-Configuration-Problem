# src/PureSAT_rebuild_solver.py
# Pure SAT optimization (REBUILD solver every iteration)
# SAT(F1 ∧ (F0 ≤ k)) with k tightened; but solver is recreated each loop.
...
def optimize_puresat_rebuild_solver(
...
) -> Tuple[Optional[List[int]], Optional[int], Dict[str, int]]:
    """
    Rebuild solver each iteration:
      solve SAT(F1 ∧ bound) with a fresh solver
      update bound = cost-1
    """
    ...
    while True:
        ...
        # Replace bound (do NOT accumulate old bounds)
        w_lits, weights = sat.cost_terms()
...
def main():
    ...
    t0 = time.time()
    model, opt_cost, stats = optimize_puresat_rebuild_solver(inst, solver_name=args.solver, start_ub=args.ub)
    log(f"done in {time.time() - t0:.2f}s | sat_calls={stats['sat_calls']} rebuilds={stats['rebuilds']} "
        f"tighten_steps={stats['tighten_steps']} | base_vars={stats['vars_base']} base_clauses={stats['clauses_base']}")

    if model is None:
        print("\nUNSAT: no feasible solution for F1 (or given UB too small).\n")
        return

    # decode (fresh build is OK)
    sat = IntegratedModelSAT(inst)
    sat.build_F1()
