...
def win_to_wsl_path(p: str) -> str:
    ...
    # Example: C:\Users\PC\...  ->  /mnt/c/Users/PC/...
    drive = ap[0].lower()
    rest = ap[2:].replace("\\", "/")
...
class IntegratedModelMaxSAT:
    ...
    # If you run this script inside Linux directly
            cmd = [openwbo_path]
            if extra_args:
                cmd += extra_args
            cmd += [wcnf_path]
        ...
        text = (proc.stdout or "") + "\n" + (proc.stderr or "")
        with open("solver_raw_eval_maxsat.log", "w", encoding="utf-8") as f:
            f.write(text)
        print("RAW output saved to solver_raw_eval_maxsat.log")
        ...
        for line in text.splitlines():
            ...
            if line.startswith("s "):
                ...
                # Try bitstring first
                only01 = re.sub(r"[01\s]", "", payload) == ""
                if only01:
                    bitstr += "".join(ch for ch in payload if ch in "01")
                else:
                    # Old/literal style: v -1 2 -3 ...
                    for p in re.findall(r"-?\d+", payload):
                        try:
                            lits.append(int(p))
                        except Exception:
                            pass
        ...
        if status and ("UNSAT" in status.upper()):
            return None
        if not costs:
            raise RuntimeError("EvalMaxSAT output missing objective line 'o <cost>'.")

        opt_cost = costs[-1]  # EvalMaxSAT có thể in nhiều dòng o; lấy dòng cuối :contentReference[oaicite:1]{index=1}

        model_set: set[int] = set()
        if bitstr:
            upto = min(len(bitstr), self.vpool.top)
            for i in range(1, upto + 1):
                if bitstr[i - 1] == "1":
                    model_set.add(i)
        else:
            # fallback: literal list
            for l in lits:
                if l > 0:
                    model_set.add(l)
...
def run_one(path: str, solver_path: str, solver_args: Optional[List[str]] = None):
    ...
    try:
        ...
        log(f"  solving EvalMaxSAT: {solver_path} ...")
        t_solve = time.time()
...
def main():
    ...
    # giữ tên option cũ để bạn khỏi đổi command
    ap.add_argument("--openwbo", default=None, help="path to EvalMaxSAT_bin (yes, still named --openwbo for compatibility)")
    ap.add_argument(
        "--openwbo_args",
        nargs=argparse.REMAINDER,
        default=[],
        help="extra args for EvalMaxSAT_bin, e.g. --openwbo_args --TCT 3600"
    )
    args = ap.parse_args()
    ...
    # default solver path: src/EvalMaxSAT_bin if exists
    solver_path = args.openwbo
    if not solver_path:
        solver_path = os.path.join(script_dir, "EvalMaxSAT_bin")
    ...
    # on Windows, if missing, try add .exe
    if not os.path.exists(solver_path) and os.name == "nt" and not solver_path.lower().endswith(".exe"):
        if os.path.exists(solver_path + ".exe"):
            solver_path = solver_path + ".exe"

    if not os.path.exists(solver_path):
        raise FileNotFoundError(f"EvalMaxSAT binary not found: {solver_path}")
