...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description="Hybrid orchestrator: Gurobi main + SAT certifier")
    ...
    mip_cmd = [
        sys.executable,
        str(src_dir / "hybrid_mip_gurobi_worker.py"),
        "--inst", args.inst,
    ...
    if args.quiet_mip:
        mip_cmd += ["--quiet"]
    if args.branch_on_dual:
        mip_cmd += ["--branch-on-dual"]
    ...
    def _add_total(st):
        st["total_runtime_sec"] = total_runtime_sec
