...
def log(msg: str) -> None:
    print(f"[SAT {time.strftime('%H:%M:%S')}] {msg}", flush=True)


def main() -> None:
    t_total0 = time.perf_counter()

    ap = argparse.ArgumentParser(description="Hybrid SAT certifier worker for RCP")
    ...
    with Solver(name=args.solver, bootstrap_with=sat.cnf.clauses, use_timer=True) as s:
        while True:
            ...
            if status != "RUNNING":
                ...
                log(f"stop because global status={status}")
                return
            ...
            if target < 0:
                ...
                log(f"OPTIMAL by SAT trivial certify | ub={ub}")
                return

            log(f"certifying current UB={ub} by solving cost <= {target}")
            ...
            if res is None:
                ...
                log("time limit while certifying")
                return
            ...
            if res is False:
                ...
                if current_ub == ub:
                    ...
                    log(f"UNSAT for outdated UB={ub}, but global_ub changed to {current_ub}; continue")
                    continue
...
#                     "sat_status",
#                     f"STOPPED_{status}",
#                     extra={"sat_solver_time_sec": s.time_accum()},
#                 )
#                 log(f"stop because global status={status}")
#                 return
...
#                     ub,
#                     source="SAT",
#                     extra={"sat_status": "OPTIMAL", "sat_solver_time_sec": s.time_accum()},
#                 )
#                 log(f"OPTIMAL by SAT trivial certify | ub={ub}")
#                 return
...
#             if res is False:
#                 # Re-read state to avoid certifying an outdated UB if MIP/SAT improved it meanwhile.
#                 st_after = read_state(args.state_file)
#                 current_ub = st_after.get("global_ub", None)
#                 if current_ub == ub:
#                     mark_optimal(
...
#                             "sat_status": "OPTIMAL",
#                             "sat_certified_against": target,
#                             "sat_solver_time_sec": s.time_accum(),
#                         },
#                     )
#                     log(f"OPTIMAL by SAT certificate | no solution with cost <= {target}")
#                     return
...
#             # SAT: found a better feasible solution than current UB
#             model = s.get_model()
#             new_cost = sat.model_cost(model)
#             update_ub(
#                 args.state_file,
